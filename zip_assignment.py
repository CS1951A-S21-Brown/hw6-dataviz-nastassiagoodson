...
def main():
    ...
    for must_have_file in ["index.html", "main.js", "main.css"]:
        if find(must_have_file, curdir) == None:
            ...
            print("We cannot find the file {} in the directory that you are".format(
                must_have_file))
            print(
                "executing this script from. Please use command 'cd <path>' to change to the right")
            print(
                "directory that contains zip_assignment.py and execute this script again.")
            sys.exit()
    ...
    if not os.path.exists(PATH_TO_SUBMISSIONLINK):
        ...
        # or len(repo_name_splitted) < 3:
        if repo_name_splitted[0] != "hw6" or repo_name_splitted[1] != "dataviz":
        ...
        correct_path = "https://cs1951a-s21-brown.github.io/{}/".format(
            repo_name)
    ...
    try:
        for sq in sequence:
            ...
            if ret != 0:
                print("Error calling git commands to push your code",
                      "EXIT CODE: ", ret)
    ...
    if not os.path.exists(PATH_TO_SUBMISSIONLINK):
        print(
            "Ruh roh. Something went wrong with zip_assignment.py. Please contact the HTAs.")
